...
function Booking() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name || formData.name.length < 2) {
      newErrors.name = 'Ім’я обов’язкове і має містити щонайменше 2 символи';
    }
    if (!formData.phone || !/^\+?\d{10,12}$/.test(formData.phone)) {
      newErrors.phone = 'Введіть коректний номер телефону (10–12 цифр, можна з +)';
    }
    if (!formData.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
      newErrors.email = 'Введіть коректний email';
    }
    if (selectedSeats.length === 0) {
      newErrors.seats = 'Виберіть щонайменше одне місце';
    }
    setErrors(newErrors);
